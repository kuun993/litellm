...
def test_resolve_api_base_defaults_to_region_host(monkeypatch):
    monkeypatch.delenv("DASHSCOPE_REALTIME_API_BASE", raising=False)
...
def test_resolve_api_base_env_override(monkeypatch):
    monkeypatch.setenv("DASHSCOPE_REALTIME_API_BASE", "wss://custom.example.com")
    assert resolve_dashscope_realtime_api_base(None, None) == "wss://custom.example.com"
...
@pytest.mark.asyncio
async def test_rejected_handshake_reaches_the_client_as_the_upstream_status():
    """A rejected DashScope handshake must close the client with the upstream status.

    websockets >= 15 raises ``InvalidStatus`` for a rejected handshake. The base
    ``OpenAIRealtime`` handler catches only the legacy ``InvalidStatusCode``, which is a
    different class, so without the override DashScope's 401 reaches the client as a 1011.
    """
    ...
    assert client_ws.close_code == 401
