...
def resolve_dashscope_realtime_api_base(api_base: str | None, dynamic_api_base: str | None = None) -> str:
    ...
    base that ``get_llm_provider`` returns is swapped for the region realtime host, which
    the ``DASHSCOPE_REALTIME_API_BASE`` env var can still override.
    """
    for candidate in (api_base, dynamic_api_base):
        if candidate and candidate.rstrip("/") != _DASHSCOPE_CHAT_API_BASE:
            return candidate
    env_api_base: Final = get_secret_str("DASHSCOPE_REALTIME_API_BASE")
    return env_api_base or DASHSCOPE_REALTIME_API_BASE
...
class DashScopeRealtime(OpenAIRealtime):
    ...
    def _handshake_status_errors(self) -> tuple[type[BaseException], ...]:
        """A rejected DashScope handshake raises ``InvalidStatus``.

        ``OpenAIRealtime`` catches ``websockets.exceptions.InvalidStatusCode``, but that is
        ``websockets.legacy.exceptions.InvalidStatusCode``, a different class from the
        ``InvalidStatus`` that websockets >= 15 raises for a rejected client handshake.
        Leaving it uncaught sends the rejection to the generic handler, so a 401 from
        DashScope reaches the client as a 1011.
        """
        import websockets
